...
public class PlaylistWindow {
     ...
     * @param playlistCalculator a playlist calculator instance to pull queue and
playlists from
     */
...
represent the current status
    ...
    private void drawPhone(int x, int y, int playlistIndex) {
        Shape phone = new Shape(x, y, PLAYLIST_SIZE, (int) (PLAYLIST_SIZE * 1.25),
            PLAYLIST_COLORS[3]);
        playlistShapes[playlistIndex] = phone;
        Shape innerPhone = new Shape(x + 5, y + 5, PLAYLIST_SIZE - 10, (int)
            (PLAYLIST_SIZE * 1.25) - 10, Color.darkGray);
        ...
        TextShape playlistName = new TextShape(textX, textY, message, Color.WHITE,
            20);
...
rectangle and a circle)
    ...
    private Shape[] drawNote(int x, int y, Color color) {
        ...
        Shape circle = new CircleShape(x - MUSIC_NOTE_SIZE + bar.getWidth() + 1, y
            + bar.getHeight() - (MUSIC_NOTE_SIZE / 2), MUSIC_NOTE_SIZE, color);
        note[0] = bar;
        note[1] = circle;
        window.addShape(bar);
...
song
    ...
    private void updateSongText() {
        ...
        message = "Suggested Playlist: " + (song.getPlaylistName().equals("") ?
            "No-Playlist" : song.getPlaylistName());
        songText = addTextShape(message, x, y);
        maxX = Math.max(maxX, x + songText.getWidth());
    ...
    private void updatePlaylistText() {
        for (int i = 0; i < playlistShapes.length; i++) {
            ...
            popText.setX(x + (playlistShape.getWidth() / 2) - (popText.getWidth() /
                2));
            ...
            rockText.setX(x + (playlistShape.getWidth() / 2) - (rockText.getWidth()
                / 2));
            ...
            countryText.setX(x + (playlistShape.getWidth() / 2) -
